...
def criar_mesa():
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO MESA (ID_MESA, NUMERO_MESA, ID_STATUS)
            VALUES (GEN_ID(GEN_MESA_ID, 1), ?, 1)
        """, (numero,))
        con.commit()
...
def criar_pedido():
    ...
    try:
        ...
        itens = dados.get('itens')  # lista de {id_produto, quantidade}
        ...
        cur.execute("""
            INSERT INTO PEDIDO (ID_PEDIDO, ID_MESA, DATA_HORA, VALOR_TOTAL, OBSERVACOES)
            VALUES (GEN_ID(GEN_PEDIDO_ID, 1), ?, CURRENT_TIMESTAMP, ?, ?)
            RETURNING ID_PEDIDO
        ...
        for item in itens_completos:
            cur.execute("""
                INSERT INTO ITEM_PEDIDO (ID_ITEM, ID_PEDIDO, ID_PRODUTO, QUANTIDADE, PRECO_UNITARIO, SUBTOTAL)
                VALUES (GEN_ID(GEN_ITEM_PEDIDO_ID, 1), ?, ?, ?, ?, ?)
            """, (id_pedido, item['id_produto'], item['quantidade'], item['preco_unitario'], item['subtotal']))

        # Atualiza status da mesa para ocupada (ID_STATUS = 2)
